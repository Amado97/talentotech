...
const db = createClient(SUPABASE_URL, SUPABASE_ANON, {
  auth: {
    ...
    persistSession: true,
    detectSessionInUrl: true,  // maneja el redirect de verificación de correo
  }
});
...
async function initApp() {
  // 1. Cargar comentarios iniciales (muestra el skeleton/loading)
  await loadComments();

  // 2. Escuchar cambios de autenticación (se dispara al inicio también)
  db.auth.onAuthStateChange(async (event, session) => {
    ...
    if (session?.user) {
      ...
      // Actualización rápida de UI para evitar parpadeo
      updateNavUI(true);
      updateCommentFormUI(true);
      
      // Carga de datos pesados en segundo plano
      await loadProfile();
      updateNavUI(true); // Re-actualiza con el nombre del perfil si existe
      updateCommentFormUI(true);
      
      await loadUserLikes();
      await loadComments(); // Recarga para mostrar botones de borrar/likes según el usuario
    } else {
      currentUser    = null;
      currentProfile = null;
      userLikes      = new Set();
      ...
      // No necesitamos recargar comentarios si ya se cargaron al inicio
    }

    // Manejo de redirección post-confirmación de email
    if (event === 'SIGNED_IN' && window.location.hash.includes('access_token')) {
      showToast('¡Email verificado! Ya puedes comentar.', 'success');
      history.replaceState(null, '', window.location.pathname + window.location.search);
    }
  });

  // 3. Verificación manual inmediata por si el listener tarda
...
async function loadComments() {
  ...
  const { data: comments, error } = await db
    .from('comments')
    .select(`
      id, name, content, created_at, user_id,
      comment_likes ( count )
    `)
